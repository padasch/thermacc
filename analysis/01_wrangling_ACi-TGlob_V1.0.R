...
df_battaglia_43147 <-
  df_tmp %>%
  dplyr::filter(sitename == "battaglia_43147") %>%
  unnest(data) %>%
...
df_carter_1866 <-
  df_tmp %>%
  dplyr::filter(sitename == "carter_1866") %>%
  unnest(data) %>%
  mutate(sample_date = dmy(Date)) %>% 
  nest(data = !all_of(c("sitename")))  
...
df_cavaleri_1866 <- 
  df_tmp %>%
  dplyr::filter(sitename == "cavaleri_1866") %>%
  unnest(data) %>% 
...
df_cernusak_14131 <-
  df_tmp %>%
  dplyr::filter(sitename == "cernusak_14131") %>%
  unnest(data) %>%
  mutate(sample_date = dmy(Date)) %>% 
  nest(data = !all_of(c("sitename")))

# Dates missing and not findable in any document on photom repository or referenced publication
df_crous_34151 <- 
  df_tmp %>%
  dplyr::filter(sitename == "crous_34151") %>%
  unnest(data) %>%
  mutate(sample_date = dmy(Date)) %>% 
  nest(data = !all_of(c("sitename")))
...
df_ellsworth_3679 <- 
  df_tmp %>%
  dplyr::filter(sitename == "ellsworth_3679") %>%
  unnest(data) %>% 
...
df_han_35139 <- 
  df_tmp %>%
  dplyr::filter(sitename == "han_35139") %>%
  unnest(data) %>%
  mutate(
    sample_date = Date,
...
df_han_36140 <- 
  df_tmp %>%
  dplyr::filter(sitename == "han_36140") %>% 
  unnest(data) %>% 
  mutate(
    # Entries like "DD-MM" are interpreted as in year 2004 (based on original ref)
    sample_date = Date,
    sample_date = ifelse(str_detect(Date, "04-May"), paste0("2004-05-04"), sample_date),
    sample_date = ifelse(str_detect(Date, "04-Oct"), paste0("2004-10-04"), sample_date),
  ) %>% 
  nest(data = !all_of(c("sitename")))

df_hikosaka_43142  <-
  df_tmp %>%
  dplyr::filter(sitename == "hikosaka_43142") %>%
  unnest(data) %>% 
...
df_jensen_4893 <-
  df_tmp %>%
  dplyr::filter(sitename == "jensen_4893") %>% 
  unnest(data) %>% 
...
df_kelly_16145  <-
  df_tmp %>%
  dplyr::filter(sitename == "kelly_16145") %>%
  unnest(data) %>% 
...
df_medlyn_355 <-
  df_tmp %>%
  dplyr::filter(sitename == "medlyn_355") %>% 
  unnest(data) %>% 
  mutate(sample_date = dmy(Date)) %>% 
  nest(data = !all_of(c("sitename")))
...
df_medlyn_36148 <-
  df_tmp %>%
  dplyr::filter(sitename == "medlyn_36148") %>% 
  unnest(data) %>% 
  mutate(sample_date = dmy(Date)) %>% 
  nest(data = !all_of(c("sitename")))
...
df_medlyn_441 <-
  df_tmp %>%
  dplyr::filter(sitename == "medlyn_441") %>% 
  unnest(data) %>% 
  mutate(sample_date = dmy(Date)) %>% 
  nest(data = !all_of(c("sitename")))
...
df_onoda_41141     <-
  df_tmp %>%
  dplyr::filter(sitename == "onoda_41141") %>% 
  unnest(data) %>% 
...
df_rogers_71157     <-
  df_tmp %>%
  dplyr::filter(sitename == "rogers_71157") %>% 
  unnest(data) %>% 
...
df_slot_980 <-
  df_tmp %>%
  dplyr::filter(sitename == "slot_980") %>%
  unnest(data) %>%
  mutate(sample_date = dmy(Date)) %>% 
  nest(data = !all_of(c("sitename")))
...
df_tarvainen_5812  <- 
  df_tmp %>%
  dplyr::filter(sitename == "tarvainen_5812") %>% 
  unnest(data) %>% 
  # Date reconstruction very vague, only reported as period in K19 data
  mutate(sample_date = Date,
    # Entries like "10-Jun" are interpreted as dd-Mon in year 2009
    sample_date = ifelse(str_detect(Date, "^[0-9]{1,2}-[A-Za-z]{3}$"), paste0(Date, "-2009"), sample_date),
    ...
    sample_date = as.Date(suppressWarnings(parse_date_time(sample_date, orders = c("Y-m-d", "d-b-Y"))))) %>% 
  nest(data = !all_of(c("sitename")))

df_tarvainen_6420  <- 
  df_tmp %>%
  dplyr::filter(sitename == "tarvainen_6420") %>% 
  unnest(data) %>% 
  mutate(sample_date = dmy(Date)) %>% 
  nest(data = !all_of(c("sitename")))
...
df_togashi_30121  <- 
  df_tmp %>%
  dplyr::filter(sitename == "togashi_30121") %>% 
  unnest(data) %>% 
...
df_tribuzy_360  <- 
  df_tmp %>%
  dplyr::filter(sitename == "tribuzy_360") %>% 
  unnest(data) %>% 
  mutate(sample_date = dmy(Date)) %>% 
  nest(data = !all_of(c("sitename")))
...
df_wang_6331  <- 
  df_tmp %>%
  dplyr::filter(sitename == "wang_6331") %>% 
  unnest(data) %>% 
...
df_tmp_date <-
  rbind(
    ...
    df_tribuzy_360,     
    df_wang_6331) %>%
  # Ensure a consistent date type across nested tibbles before unnesting.
  mutate(data = purrr::map(data, ~mutate(.x, sample_date = as.Date(sample_date))))
...
df_tmp_date_flt <-
  ...
  dplyr::filter(
    ...
    between(CO2S, ca_low, ca_upp) |
      ...
      # Case 2 - only Ci reported
      (
        is.na(CO2S) & is.na(CO2R) &
          between(Ci, ci_low, ci_upp)
      )
  )

### Extraction ----
fit_site_topt <- function(dat,
                          sitename_pattern,
                          random_effect,
                          qc_fn = identity,
                          pre_agg_fn = identity,
                          ...
                          agg_scale = "weekly") {
  dat %>%
    dplyr::filter(str_detect(sitename, sitename_pattern)) %>%
    qc_fn() %>%
    pre_agg_fn() %>%
    mutate(agg_date = agg_fun(sample_date, unit = agg_unit)) %>%
    nest(data_org = !any_of(c("sitename", "agg_date"))) %>%
    mutate(
      fit_opt = purrr::map(data_org, ~fit_nonlinear_topt(dat = .x, x = "Tleaf", y = "Photo", random = random_effect)),
      fit_opt = purrr::map(fit_opt, ~mutate(., agg_scale = agg_scale))
    )
}

site_fit_specs <- tibble::tibble(
  sitename_pattern = c(
    "battaglia_43147", "carter_1866", "cavaleri_1866", "cernusak_14131", "crous_34151",
    "ellsworth_3679", "han_35139", "han_36140", "hikosaka_43142", "jensen_4893",
    "kelly_16145", "medlyn_355xxx", "medlyn_36148", "medlyn_441|medlyn_355", "onoda_41141",
    "rogers_71157", "slot_980", "tarvainen_5812", "tarvainen_6420", "togashi_30121",
    "tribuzy_360", "wang_6331"
  ),
  random_effect = c(
    "Species", "Species", "Species", "replicate", "replicate",
    "replicate", "replicate", "replicate", "replicate", "replicate",
    "replicate", "Species", "replicate", "Species", "Species",
    "Species", "Species", "replicate", "replicate", "Species",
    "Species", "Species"
  ),
  qc_fn = list(
    identity,
    identity,
    identity,
    function(x) x %>% dplyr::filter(between(CO2R, 380, 440), PARi > 1800),
    function(x) x %>% dplyr::filter(between(CO2R, 380, 440)),
    function(x) x %>% dplyr::filter(between(CO2S, 340, 370)),
    function(x) x %>% dplyr::filter(between(CO2S, 330, 350)),
    function(x) x %>% dplyr::filter(between(Ci, 175, 230)),
    function(x) x %>% dplyr::filter(between(CO2S, 360, 375), !(Curve_Id %in% c(108:110, 115:117))),
    function(x) x %>% dplyr::filter(between(CO2R, 395, 410)),
    function(x) x %>% dplyr::filter(between(CO2R, 375, 425), Ci > 175),
    # Kept intentionally empty: medlyn_355 records are merged into medlyn_441 below.
    function(x) x %>% dplyr::filter(between(Ci, 175, 350), Curve_Id != 62),
    function(x) x %>% dplyr::filter(PARi > 1200, Cond > 0.09),
    function(x) x %>% dplyr::filter(between(Ci, 175, 350), Curve_Id != 62),
    function(x) x %>% dplyr::filter(between(Ci, 250, 350), Photo < 8.2),
    function(x) x %>% dplyr::filter(between(CO2S, 370, 410)),
    identity,
    function(x) x %>% dplyr::filter(between(CO2R, 395, 405)),
    function(x) x %>% dplyr::filter(between(CO2R, 395, 405)),
    function(x) x %>% dplyr::filter(between(CO2R, 365, 435), !(Curve_Id %in% c(19, 23, 27, 44))),
    function(x) x %>% dplyr::filter(between(CO2R, 365, 435), !(Curve_Id %in% c(19, 23, 27, 44))),
    function(x) x %>% dplyr::filter(between(Ci, 240, 400))
  ),
  pre_agg_fn = list(
  ...
  agg_fun = list(
    floor_date, floor_date, floor_date, floor_date, floor_date,
    floor_date, floor_date, floor_date, floor_date, floor_date,
    floor_date, floor_date, floor_date, floor_date, floor_date,
    round_date,
    floor_date,
...
df_pre_qc <-
  ...
  mutate(
    fit_df = purrr::pmap(
      list(sitename_pattern, random_effect, qc_fn, pre_agg_fn, agg_fun),
      \(sitename_pattern, random_effect, qc_fn, pre_agg_fn, agg_fun) {
        fit_site_topt(
          dat = df_tmp_date_flt,
          sitename_pattern = sitename_pattern,
          random_effect = random_effect,
          qc_fn = qc_fn,
          pre_agg_fn = pre_agg_fn,
          agg_fun = agg_fun
        )
      }
    )
...
if (!dir.exists(dir_tmp)) dir.create(dir_tmp, recursive = TRUE, showWarnings = FALSE)
